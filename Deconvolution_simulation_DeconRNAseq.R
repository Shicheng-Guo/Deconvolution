...
cxc <- ggplot(mtcars, aes(x = factor(cyl),fill=factor(cyl))) +  geom_bar(width = 1, colour = "blue")
cxc + coord_polar()
...
topgsi<-TopGSIByCategory(Colon$GSI,top=100)
...
signatures<-AverageWithGroup(data1_ref)  
topgsi<-TopGSIByCategory(Normal$GSI,top=100)
...
lines(x = c(0,1), y = c(0,1))
output
save.image(file="NormalPlasma-Deconvolution.RData")
...
data1_ref=Normal$ref
signatures<-AverageWithGroup(data1_ref)  
topgsi<-TopGSIByCategory(Normal$GSI,top=50)
head(Normal$GSI)
...
topgsi<-TopGSIByCategory(Normal$GSI,top=5)
head(Normal$GSI)
